...
- 128 offline tests, CI, Render.com deployment

### Phase 3.5: Postgres Migration + Scheduling Reliability (Free Tier)
**Status:** Complete · **Cost:** $0/month

- Migrated from SQLite to dual-mode Postgres/SQLite persistence (`backend/database.py`)
- Neon Postgres free tier (0.5 GB, scale-to-zero, PgBouncer pooler) for production
- SQLite fallback for local development (no DATABASE_URL needed)
- Replaced APScheduler (dies when Render sleeps) with GitHub Actions cron scheduling
- Position check every 10 min during market hours via `scheduled-signals.yml`
- Weekly keepalive workflow prevents 60-day GitHub Actions auto-disable
- CI tests against both SQLite and Postgres (service container)
- Removed persistent disk dependency from Render deployment
- Automated stop-loss/take-profit exit logic with volatility-scaled bands (2:1 reward:risk)
