...
| Execution | Paper trading orders, alpha decay IC half-life, GitHub Actions scheduled cron, Render.com deploy, Postgres/SQLite dual-mode persistence | `execution/__init__.py`, `analysis/alpha_decay.py`, `render.yaml`, `backend/database.py` |
...
`backend/database.py` supports **dual-mode** persistence: Postgres (Neon free tier) when `DATABASE_URL` is set, SQLite when it is not. All queries use raw SQL with a thin abstraction layer for placeholder syntax (`%s` vs `?`) and ID generation (`RETURNING id` vs `lastrowid`).
...
intraday_signals      (id, saved_at, ticker, signal, mean_ic, sharpe, sortino,
                       ...
                       -- SEM: ic_sem, sharpe_sem, hit_rate_sem
                       equity_dates_json, latest_signal)
-- execution
paper_trades          (id, ticker, signal, qty, order_id, status,
                       filled_price, submitted_at, filled_at)
open_positions        (id, ticker, side, qty, entry_price, stop_loss_price,
                       take_profit_price, opened_at, status, exit_price,
                       close_reason, closed_at)
alpha_decay           (id, computed_at, ticker, half_life_bars, ic_by_lag_json,
                       half_life_ci_5, half_life_ci_95)
```

**Indexes:** `idx_signals_ticker_recorded`, `idx_intraday_ticker`, `idx_positions_status`, `idx_trades_submitted`.

All tables are created via `CREATE TABLE IF NOT EXISTS` in `init_db()`; new columns are added non-destructively via `_migrate_columns()` (idempotent `ALTER TABLE`), so existing databases upgrade in place without data loss.
...
|-----------|------|---------|
| GitHub Actions cron (`scheduled-signals.yml`) | Production (Render) | Triggers FastAPI endpoints via HTTP — wakes sleeping backend |
| APScheduler (`SCHEDULER_ENABLED=true`) | Local development | In-process cron — works only while the FastAPI process is alive |
| Keepalive workflow (`keepalive.yml`) | Weekly (Sunday) | Commits a timestamp to prevent GitHub's 60-day auto-disable |

**Production schedule** (weekdays, ET → UTC):

| Job | ET Time | UTC Cron | Endpoint |
|-----|---------|----------|----------|
| Daily pipeline | 9:35 AM | `35 13 * * 1-5` | `POST /api/run` |
| Hourly pipeline | 10:35 AM – 4:35 PM | `35 14-20 * * 1-5` | `POST /api/intraday/run` |
| Position check | Every 10 min, 9 AM – 4 PM | `*/10 13-20 * * 1-5` | `POST /api/positions/check` |
| Nightly refresh | 9:30 PM | `30 1 * * 2-6` | `POST /api/data/refresh` |

---

## Migration Decision Record

**Problem:** SQLite on Render's persistent disk + APScheduler in-process = fragile. The DB is tied to one host, and the scheduler dies when Render's free tier spins down after 15 min idle.

**Options evaluated:**
- Database: Neon Postgres (free) vs Supabase (free, 7-day pause) vs Oracle Cloud ARM + TimescaleDB (self-hosted, Oracle halved specs June 2026 without notice)
- Scheduling: GitHub Actions cron vs external ping service vs direct-to-Postgres script

**Decision:** Neon Postgres (transparent scale-to-zero, no pause mechanism needed, no ops burden, 0.5 GB free) + GitHub Actions cron (reuses existing FastAPI endpoints, keepalive prevents 60-day disable, unlimited minutes on public repos).

**Tradeoff accepted:** ~60s cold-start chain on first cron trigger (Render wake + Neon wake). Acceptable for non-latency-sensitive daily/hourly signals computed on prior bar's close.

**Why not Render's free Postgres:** Expires after 30 days, then deleted with all data.
...
|-----------|----------|-------|
| Backend API | Render.com (free web service) | Spins down after 15 min idle |
| Frontend | Render.com (free static site) | React/Vite, served from `dist/` |
| Database | Neon Postgres (free tier) | 0.5 GB, scale-to-zero, PgBouncer pooler |
| CI/CD | GitHub Actions (free, public repo) | Tests, type checks, build |
| Scheduling | GitHub Actions (free, public repo) | Cron triggers for all pipelines |

**Environment variables** (set in Render Dashboard, not committed):
`DATABASE_URL`, `GROQ_API_KEY`, `GROQ_API_KEY_2`, `ALPACA_API_KEY`, `ALPACA_SECRET_KEY`

**GitHub Actions secrets:**
`RENDER_BACKEND_URL` (your Render backend URL, e.g. `https://alphaflow-backend.onrender.com`)
