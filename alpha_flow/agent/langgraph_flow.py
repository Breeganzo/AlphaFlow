...
def _determine_signals_crosssectional(
    ...
    # Adaptive cross-sectional book: top/bottom fraction AND ≥ z_min std-devs
    # from the cross-sectional mean, so the book shrinks in a weak/flat
    # cross-section instead of always trading a fixed count.
    from alpha_flow.config.settings import SIGNAL_CROSS_Z_MIN
    from alpha_flow.analysis.signal_classification import adaptive_rank_sets
    ofi_values = {t: snapshots[t]["ofi_zscore"] for t in snapshots}
    buy_rank, sell_rank = adaptive_rank_sets(ofi_values, SIGNAL_RANK_FRACTION, SIGNAL_CROSS_Z_MIN)
