          ...
          {resolution === 'hourly' && (
            <>
              ...
              <Card
                ...
                right={
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {tradePnl.data?.total_pnl != null && (
                      <span style={{ color: (tradePnl.data.total_pnl ?? 0) >= 0 ? S.positiveVal : S.negativeVal, fontSize: 12, fontWeight: 700 }}>
                        {(tradePnl.data.total_pnl ?? 0) >= 0 ? '+' : ''}${tradePnl.data.total_pnl.toFixed(2)} PnL
                      </span>
                    )}
              ...
              <Card
                ...
                {(() => {
                  ...
                  const { gross_equity, net_equity, long_tickers, short_tickers, long_ics, short_ics,
                          ...
                          equity_dates, position_detail } = pd
                          ...
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {(long_tickers ?? []).map((t: string, i: number) => (
                              <Tooltip key={t} content={`IC = ${((long_ics?.[i] ?? 0) * 100).toFixed(2)}%. Click for position attribution.`}>
                                <span onClick={e => setPosDetailOpen(prev => prev?.ticker === t ? null : { ticker: t, rect: (e.currentTarget as HTMLElement).getBoundingClientRect() })}
                          ...
                          <div ref={posDetailPanelRef} style={{
                            ...
                            {det ? (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: S.muted }}>Weight</span><span style={{ color: S.text, fontWeight: 700 }}>{(det.weight * 100).toFixed(1)}%</span></div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: S.muted }}>Mean IC</span><span style={{ color: S.text, fontWeight: 700 }}>{(det.mean_ic * 100).toFixed(2)}%</span></div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: S.muted }}>PnL Contribution</span><span style={{ color: det.pnl_contribution_pct >= 0 ? S.positiveVal : S.negativeVal, fontWeight: 700 }}>{det.pnl_contribution_pct >= 0 ? '+' : ''}{det.pnl_contribution_pct.toFixed(2)}%</span></div>
                              </div>
                            ) : (
