...
from backend.database import (init_db, start_run, finish_run, get_run_history,
                              ...
                              save_open_position, get_open_positions,
                              close_position_db, get_closed_positions,
...
async def lifespan(_app: "FastAPI"):  # type: ignore[name-defined]
    ...
    if os.getenv("SCHEDULER_ENABLED", "false").lower() == "true":
        try:
            ...
            def _position_check():
                """Weekdays, every POSITION_CHECK_INTERVAL_MIN minutes, market hours:
                close any open paper position whose signal has flipped or whose
                price has breached its stop-loss/take-profit band. Cheap (no
                pipeline run) — just reads existing signals + latest prices."""
                try:
                    result = check_positions()
                    if result.get("closed"):
                        print(f"[scheduler] position check closed {len(result['closed'])} position(s): {result['closed']}")
                except Exception as exc:
                    print(f"[scheduler] position check error: {exc}")
            ...
            from alpha_flow.config.settings import POSITION_CHECK_INTERVAL_MIN
            scheduler.add_job(_position_check, CronTrigger(day_of_week="mon-fri", hour="9-16", minute=f"*/{POSITION_CHECK_INTERVAL_MIN}", timezone=et), id="position_check", name=f"Position exit check (every {POSITION_CHECK_INTERVAL_MIN}min, market hours ET)")
            scheduler.start()
            _scheduler_ref = scheduler
            print("[scheduler] started — daily signals 9:35 AM ET · hourly signals 10:35 AM-4:35 PM ET · data refresh 9:30 PM ET · position checks every 5min (all weekdays, DST-aware)")
...
def delete_custom_ticker(ticker: str) -> dict:
    ...
    try:
        from backend.database import delete_signals_for_ticker
        delete_signals_for_ticker(t)
    except Exception as _e:
...
def _build_intraday_cards(results: dict) -> list[dict]:
    ...
    from alpha_flow.config.settings import (
        SIGNAL_RANK_FRACTION, SIGNAL_SIGNIFICANCE_ALPHA, SIGNAL_CROSS_Z_MIN,
    )
    from alpha_flow.analysis.signal_classification import (
        classify_signal, is_high_conviction, adaptive_rank_sets,
    )
    ...
    # Adaptive cross-sectional book, ranked by the DIRECTIONAL signal (latest
    # direction-corrected predicted return — NOT mean_ic, which is skill not
    # direction). A name trades only if it is in the top/bottom fraction AND at
    # least SIGNAL_CROSS_Z_MIN std-devs from the cross-sectional mean, so the
    # book size reflects the tradeable separation actually present each run
    # (no longer a mechanically fixed 10L/10S/30H).
    ls_values = {t: valid[t].get("latest_signal", 0.0) for t in valid}
    buy_rank, sell_rank = adaptive_rank_sets(ls_values, SIGNAL_RANK_FRACTION, SIGNAL_CROSS_Z_MIN)
    ...
    for ticker, res in valid.items():
        ...
        # Confirm only when the directional signal points the right way AND the
        # model has non-negative predictive skill for this name. mean_ic is skill
        # (direction-agnostic), so the SAME ic >= 0 gate applies to both legs —
        # we drop names where the model is anti-predictive (ic < 0) rather than
        # trusting their direction. (Requiring ic <= 0 to short would be backwards.)
        signal = classify_signal(
            signal_value=ls,
            in_buy_rank=ticker in buy_rank,
            in_sell_rank=ticker in sell_rank,
            sign_ok_buy=(ls >= 0 and ic >= 0),
            sign_ok_sell=(ls <= 0 and ic >= 0),
...
async def execute_signals() -> dict:
    ...
    """
    from backend.database import get_intraday_signals_db
    from alpha_flow.execution import submit_order, compute_stop_take_profit
    from alpha_flow.execution.position_manager import estimate_daily_return_std
    ...
    for card in cards:
        ...
        try:
            ...
            # Open a tracked position (for the automated exit job) whenever the
            # order was genuinely accepted — not for stub/error/already-skipped
            # results, which never actually created a position to manage.
            if result.get("status") in ("filled", "pending", "pending_new", "new", "accepted"):
                entry_price = result.get("filled_avg_price")
                if not entry_price:
                    from alpha_flow.execution.position_manager import get_latest_price
                    entry_price = get_latest_price(ticker)
                if entry_price:
                    vol = estimate_daily_return_std(ticker)
                    stop, take = compute_stop_take_profit(entry_price, signal, vol)
                    save_open_position(ticker, signal, result.get("qty", 10), entry_price, stop, take)
...
def list_positions() -> dict:
    """Return currently-open tracked positions + recently-closed ones
    (with the reason each was closed: signal_flip / stop_loss / take_profit / manual)."""
    return {"open": get_open_positions(), "closed": get_closed_positions(50)}


@app.post("/api/positions/check", tags=["execution"])
def check_positions() -> dict:
    """
    Run the automated exit-logic pass: for every open tracked position,
    close it if the model's latest signal has flipped against it, or if
    price has breached its stop-loss/take-profit band. Safe to call
    repeatedly/on a schedule — a position with no trigger is left untouched.
    """
    from alpha_flow.execution import close_position
    from alpha_flow.execution.position_manager import check_positions_logic, get_latest_price
    from backend.database import get_intraday_signals_db
    ...
    cards = get_intraday_signals_db() or []
    latest_signals = {c["ticker"]: c.get("signal") for c in cards}
    latest_prices = {p["ticker"]: get_latest_price(p["ticker"]) for p in positions}

    actions = check_positions_logic(positions, latest_signals, latest_prices)
    closed = []
    for action in actions:
        result = close_position(action["ticker"])
        exit_price = result.get("exit_price") or latest_prices.get(action["ticker"])
        close_position_db(action["position_id"], exit_price, action["reason"])
        closed.append({**action, "exit_price": exit_price, "alpaca_status": result.get("status")})
...
@app.get("/api/portfolio/account", tags=["execution"])
def portfolio_account() -> dict:
    """
    Live paper-account snapshot straight from Alpaca — the broker is the source
    of truth for P&L, not our local order table (which may hold unfilled orders).

    Returns account equity, today's P&L (equity − last_equity), total P&L vs the
    $100k paper starting capital, and every open position with its live
    unrealized P&L. If Alpaca keys are absent/unreachable, returns
    {"connected": false} so the UI can degrade gracefully.
    """
    ...
    try:
        ...
        if not (ALPACA_API_KEY and ALPACA_SECRET_KEY):
            return {"connected": False, "reason": "no_keys",
                    "note": "Add ALPACA_API_KEY + ALPACA_SECRET_KEY to .env to enable live P&L."}
        tc = TradingClient(ALPACA_API_KEY, ALPACA_SECRET_KEY, paper=True)
        acct = tc.get_account()
        equity = float(acct.equity)
        last_equity = float(acct.last_equity)          # equity at prior session close
        today_pl = equity - last_equity
        total_pl = equity - PAPER_START_CAPITAL

        positions = []
        for p in tc.get_all_positions():
            positions.append({
                "ticker": p.symbol,
                "side": "LONG" if str(p.side).endswith("LONG") else "SHORT",
                "qty": abs(int(float(p.qty))),
                "entry_price": round(float(p.avg_entry_price), 4),
                "current_price": round(float(p.current_price), 4),
                "market_value": round(float(p.market_value), 2),
                "unrealized_pl": round(float(p.unrealized_pl), 2),
                "unrealized_plpc": round(float(p.unrealized_plpc) * 100, 3),  # → percent
            })
        ...
        return {
            ...
            "starting_capital": PAPER_START_CAPITAL,
            "today_pl": round(today_pl, 2),
            "today_pl_pct": round((today_pl / last_equity * 100) if last_equity else 0.0, 3),
            "total_pl": round(total_pl, 2),
            "total_pl_pct": round((total_pl / PAPER_START_CAPITAL * 100), 3),
            "open_positions": len(positions),
            "positions": positions,
        }
    except Exception as exc:  # noqa: BLE001
        return {"connected": False, "reason": "error", "note": str(exc)[:200]}
...
def delete_all_trades() -> dict:
    ...
    deleted = 0
    try:
        from backend.database import delete_all_trades as _delete_all
        deleted = _delete_all()
...
def cancel_pending_trades() -> dict:
    ...
    # Mark pending rows in DB as 'cancelled'
    try:
        from backend.database import cancel_pending_trades as _cancel_pending
        cancelled_local = _cancel_pending()
    except Exception as _db_err:
        print(f"[cancel_pending] DB update error: {_db_err}")

    return {
        "cancelled_via_alpaca": bool(cancelled_api),
        "local_rows_updated": cancelled_local,
...
def get_hawkes_series(ticker: str = Query(...), n: int = Query(default=120)) -> dict:
    ...
    try:
        ...
        from alpha_flow.core.hawkes import hawkes_intensity_zscore, hawkes_branching_ratio
        series = hawkes_intensity_zscore(df).dropna().tail(n)
        branching = hawkes_branching_ratio(df["volume"].astype(float))
        return {
            "ticker": t,
            "data": [{"time": str(ts), "hawkes_z": round(float(v), 4)} for ts, v in series.items()],
            "branching_ratio": branching["branching_ratio"],
            "regime": branching["regime"],
            "note": "Hawkes intensity z-score: >1.5 = clustered trades (buy/sell burst), <−1.5 = calm period. "
                    f"Branching ratio η={branching['branching_ratio']} ({branching['regime']}) — expected follow-on events per order."
        }
    except Exception as exc:
...
def portfolio_simulate() -> dict:
    ...
    # Statistical robustness: is the Sharpe real, and does it survive the
    # multiple testing implicit in ranking every ticker to build the book?
    curve = result.get("net_equity") or result.get("gross_equity")
    if curve and len(curve) > 3:
        import numpy as _np
        from alpha_flow.analysis.performance import (
            probabilistic_sharpe_ratio, deflated_sharpe_ratio,
        )
        rets = _np.diff(_np.asarray(curve, dtype=float)) / _np.asarray(curve[:-1], dtype=float)
        rets = rets[_np.isfinite(rets)]
        n_trials = max(len(cards), 2)  # tickers screened → multiple-testing count
        result["robustness"] = {
